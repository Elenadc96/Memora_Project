      ...
      <div v-if="open" class="dialog-overlay" @click.self="close">
        <div class="dialog-panel">
          ...
          <div class="px-6 pt-5 pb-4 flex items-start justify-between flex-shrink-0">
            <div>
              <h2 class="text-lg font-semibold text-primary dark:text-on-surface">Modifica materia</h2>
              <p class="text-sm text-text-muted dark:text-on-surface/60 mt-0.5">Aggiorna le informazioni della materia</p>
            </div>
            <button class="close-btn" @click="close"><X class="w-4 h-4" /></button>
          </div>
          ...
          <div class="px-6 pb-4 space-y-4 overflow-y-auto flex-1">
            ...
            <div class="preview-card">
              <div class="w-9 h-9 rounded-xl flex items-center justify-center text-base flex-shrink-0" :style="iconStyle">
                {{ selectedEmoji }}
              </div>
...
const emit = defineEmits<{
  ...
  'subject-updated': [payload: { name: string; description: string; color: string; emoji: string }]
}>()
...
// Pre-popola i campi quando il dialog si apre con una materia
watch(() => props.subject, (s) => {
  if (!s) return
  name.value          = s.subjectName
  description.value   = s.description ?? ''
  selectedEmoji.value = s.emoji ?? '📚'
  selectedColor.value = COLOR_OPTIONS.find((c) => c.hex === s.color) ?? COLOR_OPTIONS[0]
}, { immediate: true })
...
.close-btn {
  ...
  color: var(--color-primary); opacity: 0.5; flex-shrink: 0;
...
.dialog-fade-enter-active .dialog-panel, .dialog-fade-leave-active .dialog-panel { transition: transform 0.2s, opacity 0.2s; }
.dialog-fade-enter-from, .dialog-fade-leave-to { opacity: 0; }
.dialog-fade-enter-from .dialog-panel, .dialog-fade-leave-to .dialog-panel { transform: scale(0.95) translateY(8px); opacity: 0; }
</style>
